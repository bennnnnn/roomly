#!/bin/bash
# guard-shell.sh — block dangerous shell commands for Claude Code.
# Mirrors .cursor/hooks/guard-shell.sh
#
...
deny() {
  jq -n --arg m "$1" '{
    decision: "deny",
    reason: ("BLOCKED by .claude/hooks/guard-shell.sh: " + $m)
  }'
  exit 0
}
...
ask() {
  jq -n --arg m "$1" '{
    decision: "ask",
    reason: ("Hook flagged: " + $m)
  }'
  exit 0
}

# Split cleaned command into segments on &&, ||, ;, |
segments=()
...
for seg in "${segments[@]}"; do
  # Bypass commit hooks via long flags
  ...
  # Short form -n only matters on git commit
...
if [[ "$cleaned" =~ psql.*supabase\.(co|com) ]]; then
  deny "Direct psql against a Supabase remote is forbidden. Use the Supabase MCP apply_migration / execute_sql instead."
fi
